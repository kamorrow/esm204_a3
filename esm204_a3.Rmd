...
ggplot() +
  ...
  geom_text(aes(x = 300000,
            ...
            hjust = 0,
            angle = -25) +
  geom_ribbon(data = agg %>% filter(q_agg < dmd_sq_agg),
            aes(x = q_agg,
                ymax = p_vector,
                ymin = 3),
                alpha = 0.5) +
  ...
  geom_text(aes(x = 700000,
            ...
            hjust = 0,
            angle = 10) +
    geom_ribbon(data = predict_supply %>% filter(q < dmd_sq_agg),
            aes(x = q,
                ymax = 3,
                ymin = p),
                alpha = 0.5,
...
benefit_p_sq <- integrate(supply_fun, 0, dmd_sq_agg)$value
#I think this needs to be altered too once the supply curve is calculated. It should be the triangle above the supply curve and below the market price. - EM
# word - KM
# same as (dmd_sq_agg * 3)/2
...
supply_fun_ntl_env_cost <- function(q){
  p <- supply_m * q + ntl_env_cost
  return(p)
}

predict_supply_ntl_env_cost <- data.frame(q = q_vector_sup) %>%
  mutate(p = supply_fun_ntl_env_cost(q))

## Internationally

intl_env_cost <- 0.5

supply_fun_intl_env_cost <- function(q){
  p <- supply_m * q + intl_env_cost
  return(p)
}

predict_supply_intl_env_cost <- data.frame(q = q_vector_sup) %>%
  mutate(p = supply_fun_intl_env_cost(q))
...
ggplot() +
  ...
  # supply with national cost
    geom_line(data = predict_supply_ntl_env_cost,
            aes(x = q,
                y = p),
            color = "blue") +
    geom_text(aes(x = 760000,
  ...
  # supply with international cost
    geom_line(data = predict_supply_intl_env_cost,
            aes(x = q,
                y = p),
            color = "navy") +
  geom_text(aes(x = 760000,
