...
```{r}
benefit_high_tax <- (integrate(high_p_fun, 0, dmd_sq_high)$value - (dmd_sq_high * high_q_fun(p_int_tax)))
```
...
```{r}
benefit_low_tax <- (integrate(low_p_fun, 0, dmd_sq_low)$value - (dmd_sq_low * low_q_fun(p_int_tax)))
```
...
```{r}
benefit_p_tax <- integrate(supply_fun, 0, supply_tax_q_fun(supply_tax_p_fun(q_int_tax - 1)))$value
```
