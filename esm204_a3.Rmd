...
tax_vector <- seq(0, 5, 0.01)


tax_scen <- data.frame(tax = tax_vector) %>%
  mutate(tot_dmd_q = (agg_lm$coefficients["(Intercept)"] - tax)/(supply_m - agg_lm$coefficients["q"]),
         p = agg_p_fun(tot_dmd_q),
         low_dmd_q = low_q_fun(p),
         high_dmd_q = high_q_fun(p),
         pct_low = low_dmd_q / tot_dmd_q,
         pct_high = high_dmd_q / tot_dmd_q,
         env_cost = 1.5 * tot_dmd_q,
         cs_low = ((low_lm$coefficients["(Intercept)"] - p) * low_dmd_q) / 2,
         cs_high = ((high_lm$coefficients["(Intercept)"] - p) * high_dmd_q) / 2,
         tax_revenue = tot_dmd_q * tax,
         tax_low = tax_revenue * pct_low,
         tax_high = tax_revenue * pct_high,
         env_cost = 1.5 * tot_dmd_q,
         welfare_low = cs_low + tax_low - env_cost,
         welfare_high = cs_high + tax_high,
         p_supply = supply_tax_p_fun(tot_dmd_q, tax),
         p_supply_check = supply_tax_p_fun(tot_dmd_q, tax) - tax,
         welfare_prod = (supply_tax_p_fun(tot_dmd_q, tax) * tot_dmd_q) / 2)
```
...
ggplot() +
  ...
  labs(title = "Electric Vehicle & No Gas Tax",
       subtitle = "",
       y = "Price (dollars)",
       x = "Quantity (gallons)",
       caption = "\nESM 204 Spring 2020\nElliott Matthews & Keene Morrow") +
